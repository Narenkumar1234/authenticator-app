...
// For 24-hour persistence across popup opens, the password is stored in
// chrome.storage.session — MV3's in-memory-only storage that is never written
// to disk and is cleared automatically on browser restart.
// This is the only way to re-derive the CryptoKey after the popup's JS
// context is destroyed. The tradeoff is acceptable because session storage
// lives only in RAM for the current browser session.
 ...
 * chrome.storage.session is in-memory only (never hits disk) and is
 * cleared when the browser quits, so this is safe for the 24h window.
 * @param {string} password
 */
 ...
 * Re-derives the CryptoKey from the stored password via verifyMasterPassword.
...
async function tryRestoreSession() {
  ...
  // Slow path: popup was closed, re-derive from session-stored password
